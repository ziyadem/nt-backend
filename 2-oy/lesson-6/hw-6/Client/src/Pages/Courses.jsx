...
const Courses = () => {
  ...
  const deleteCourse =async (course_id) => {
   await fetch(`http://localhost:5000/courses/${course_id}`, {
      method: "DELETE",
      headers: {
        "Copntent-Type": "application/json",
  ...
  const updateCourse= async (e) => {
    ...
    await fetch(`http://localhost:5000/courses/${upd.id}`, {
      method: "PUT",
      headers: {
      ...
      .then((res) => res.json())
      .then((data) => {
       if(data.status!==200){return  toast(data.msg,{type:"error"})}
       toast(data.msg, { type: "success" });
       useDel(Math.random() * (1000 - 1) + 1);
       console.log(Math.random() * (1000 - 1) + 1);
       title.value='';
  ...
  return (
    <>
      ...
      <section className="container courses">
        <h2 className="text-center pt-3 pb-3 fs-1">Courses</h2>
