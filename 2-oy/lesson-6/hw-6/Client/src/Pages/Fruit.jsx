...
const Fruit = () => {
  ...
  const deleteCourse = async (course_id) => {
    await fetch(`http://localhost:5000/fruit/${course_id}`, {
      method: "DELETE",
      headers: {
        "Copntent-Type": "application/json",
  ...
  const updateCourse = async (e) => {
    ...
    await fetch(`http://localhost:5000/fruit/${upd.id}`, {
      method: "PUT",
      headers: {
  ...
  const createCourse = async (e) => {
    e.preventDefault();
    let { title, color } = e.target;
    await fetch(`http://localhost:5000/fruit`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        token: localStorage.getItem("token"),
  ...
  return (
    <>
      ...
      <section className="container courses">
        <h2 className="text-center pt-3 pb-3 fs-1">Fruits</h2>
